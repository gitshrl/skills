...
When a chain is needed, the workflow order is: drill (fuzzy plan) → domain-model (terms worth pinning; user-only) → implement (settled plan; debug owns anything that breaks along the way) → verify (before claiming done) → architecture (existing code fights you). Session utilities sit outside the chain: handoff, research, teach (user-only), land, parallel, delegate, write-skill.
