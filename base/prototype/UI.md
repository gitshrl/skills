...
Once a variant has won, write down which one and why somewhere durable (commit message, ADR, issue, or a `NOTES.md` next to the prototype); an ADR only when the answer passes the three-part test (hard to reverse, surprising without context, the result of a real trade-off). See the When done section of [SKILL.md](SKILL.md). Then:
